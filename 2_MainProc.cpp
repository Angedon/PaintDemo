...
LRESULT CALLBACK MainProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case ID_FILE_OPEN:
		{
			//PostMessage(hwndCreate, WM_DESTROY, 0, 0);
			//ShowWindow(hwndCreate, SW_HIDE);
			//MessageBox(hwnd, L"Open", L"INFO", MB_OK);
			//GetOpenFileName(&ofn);
			...
			GetOpenFileName(&ofn);
		//	HBITMAP hBitmap = (HBITMAP)LoadBitmap(NULL, ofn.lpstrFile , IMAGE_BITMAP , 0, 0, LR_LOADFROMFILE);
			HBITMAP hBitmap  = LoadBitmap(NULL, ofn.lpstrFile);
			BITMAP Bitmap;
		...
		/*	hwndCreate = CreateWindowEx(0, L"Create", L"Новый рисунок",
			...
			hwndCanvas = CreateWindowEx(0, L"Canvas", (LPCTSTR)NULL,
				WS_CHILD, 0, 0,
				Bitmap.bmWidth, Bitmap.bmHeight, HWND_WorkProc, NULL, MainH, NULL);
		
			//SendMessage(HWND_WorkProc_2, WM_COPYDATA, (WPARAM)hwndCanvas, NULL);
			resizeWorkProc = true;
			//update = true;
			Canvas* temp;
			temp = Canvas::AddCan(hwndCanvas, Bitmap.bmWidth, Bitmap.bmHeight,true);
		  
			newHwnd = hwndCanvas;
			SetFocus(hwndCanvas);
			//can = Canvas::Search(hwndCanvas);
			HDC veryNewHwnd = GetWindowDC(hwndCanvas);
			RECT rect;
			GetClientRect(hwndCanvas, &rect);
			temp->hdcMem = CreateCompatibleDC(veryNewHwnd);
			//hbmMem = CreateCompatibleBitmap(veryNewHwnd, rectC.right, rectC.bottom);
			//SelectObject(can->hdcMem, hbmMem);
		...
		//	hOld = SelectObject(temp->hdcMem, hbmMem);
			BitBlt(temp->hdcMem, 0,0, Bitmap.bmWidth, Bitmap.bmHeight, hCompatibleDC, 0, 0, SRCCOPY);
